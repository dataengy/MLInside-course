...
- Разбор `MLInside_Введение-в-dbt_v3.15 (1).pptx` против собранного 3.15: правки системны
  (буллеты → кегль мастера, визуал к нижней кромке 7.01″, таблицы на 2.47″, колонка
  буллетов адаптивная, пустые плейсхолдеры сняты, обводка код-панелей 2419FF → тёмная) и
  потому переехали в **профиль генератора** `alina-2026-08` (`settings/formats.yml`), а не в
  один `.pptx` — иначе следующий `just build` их бы затёр. Решения по 6 правилам (R1, R2, R3,
  R4, R6, R11) — `accept`; R7 (капслок титула) — `reject`, решение вкуса, не запрошено.
- Три регрессии пересохранения через Google Slides (тема Corbel → Calibri, потерянные
  заметки титула, склейка абзацев на слайде 23) в мерж НЕ взяты и названы в отчёте
  (`docs/reviews/merge/*.md`).
- Новая лента `src/preza_merge/` (`propose → решения человека → apply → verify`),
  `settings/merge.yml`, `settings/formats.yml`. Спека: [docs/preza-merge-lane.md](preza-merge-lane.md).
- Нотация версии расширена до `x.y.z[+descr]`; результат — `v3.19.1+alina-fmt` (70 слайдов).
  Независимая проверка собранного файла: обводка `2419FF` отсутствует, нижние кромки визуала
  на 7.01″, верх таблиц на 2.47″, явный кегль тела — 0 (весь явный кегль, что остался, — в
  код-панелях и текстбоксах, вне области действия R1).
- `just preza-merge-verify` на этом кейсе остаётся красным — но по измеренной причине, не по
  той, что была записана здесь раньше. Первая версия сверки собирала `base`-контент БЕЗ
  `deck.format` (историческая yaml из `--base-content-rev` старше самого ключа → `settings.load`
  откатывался на `classic`) и сравнивала НЕотформатированную пересборку с формой ревьюера — это
  давало 110 расхождений и 40 «разошедшихся по шрифту» слайдов, из которых 33 были артефактом
  самой ошибки сверки, а не недочётом правил. После починки (пересборка несёт `deck.format` из
  предложения; проверка R1 смотрит только на `runs_size_cleared`, а не на весь класс `font` —
  R5, намеренная ручная усадка шрифта в укороченных код-панелях, тоже даёт класс `font`, но не
  признак того, что R1 не отработало) шрифтовых расхождений — 0, слайдов «по шрифту» — 0.
  Допуски `verify.tolerances` (были единым `0.4″` без измерений) разнесены по атрибутам:
  `left/top/height = 0.45″` — туго, это края, которые R2/R3 обязаны ставить точно (шум на
  форме, которую ни одно правило не двигает, — верх/лево таблиц — <0.1″); `width = 5.5″` —
  широко намеренно: R4 выбирает БИНАРНО между узкой 6.2″ и полной 12.3″ колонкой, а ревьюер
  подбирала ширину непрерывно 6.2–11.6″ (измерено: 28 слайдов, медиана 0.99″, максимум 5.40″ —
  задокументированное приближение правила, не дефект). С этими допусками сверка находит 45
  остаточных расхождений на 19 слайдах (top 27 · height 14 · left 4 · width 0) — все на
  код-панелях, их подписях-textbox и картинках слайдов 14, 19, 21, 22, 24, 28, 29, 30, 31, 32,
  34, 35, 37, 41, 44, 46, 49, 55, 56: R2 привязывает нижнюю кромку каждой панели к единому
  `visual_bottom` (7.01″), приближающему реальный разброс 6.66–7.73″, а автоусадка код-панели
  (`_fit_code_size`) не всегда попадает в высоту, которую подобрала ревьюер вручную — расхождение
  каскадом уходит в top/height; плюс одна табличная высота (слайд 56, «ДЗ: критерии оценки»,
  1.65″→2.91″ — высоту таблиц ни одно правило не двигает). Это зафиксированный недостаток
  приближения R2/R3, не повод раздвигать допуск дальше — допуски НЕ подгонялись под прохождение.
  Инвариантный слой чист: слайдов 57↔57, ссылок 81↔81, футеров «📚 Материалы» 43↔43, заметок
  70↔70.
- Пин-тест `test_dbt_deck_uses_the_merged_format_profile` (`src/tests/test_content.py`)
  закрепляет `format_name == "alina-2026-08"`.
## 2026-08-26 — Дека чужого предмета выведена из-под наших правок ([#7](https://github.com/dataengy/MLInside-course/issues/7))

- Apache-Airflow-дека — предмет Влада Бояджи, её содержание ведёт он. Новый ключ
  `settings/config.yml → deck_generation.editing_excluded` фиксирует это как настройку,
  а не как договорённость в чате.
- Запрет стоит на **единственной точке записи** контента
  (`scripts/preza/edit_slides.py::_write`), поэтому его нельзя обойти, выбрав другую
  команду редактора; `list`/`extract` (чтение) работают как раньше. Настройки читаются
  fail-loud: битый или пропавший `config.yml` роняет команду, а не снимает запрет —
  иначе защита исчезала бы молча.
- Читающие ленты не затронуты намеренно: `preza-review`, планы блоков записи и
  публикация (`just publish-new`) деку по-прежнему видят. Исключение про правку, а не про
  существование — дека остаётся в расписании и уже опубликована (v1.1).
- Инвариант 10 сабагента `preza-accents-keeper` и раздел «Кто есть кто» в
  `docs/course-rules.md`: пробел в чужой деке — сообщение владельцу, а не патч (агент
  правит файлы напрямую, мимо редактора, поэтому одного кода мало).
...
- **Источник — чат с менеджером курса** (Алина Веденская, менеджер образовательных проектов
  MLInside, куратор) и Владом Бояджи: дизайн ок и финальный, тайминг 50–90 мин без ограничений,
  **монтаж режет запись на уроки ≤25 мин → паузы между блоками**, слайды без лимита,
  капслок / код-блоки / «доп. материалы» — ок, **не склеивать** записи, тест-клип 1–2 мин,
  светлый фон без окна, **всё записать в августе** (анонс — сентябрь), ДЗ не проверяются
  (балльная система — открытый вопрос).
- `settings/config.yml`: `participants` (Алина — менеджер/куратор, @alina_3V; Влад — GitHub
  @vboyadzhi) + новая секция **`course_production`** — SSoT скаляров правил (дедлайны, лимит
  блока, мин/слайд, дизайн, запись, ДЗ, pipeline). Читается fail-loud, без дефолтов.
- **`docs/course-rules.md`** (роли, пайплайн лекции, правила, планы блоков, статус нашей
  стороны, фидбек-лист по dbt-деке) и **`docs/course-qa.md`** (16 отвеченных Q&A + 5 открытых
  чекбоксами в разделе «Открытые вопросы»).
- **`src/course`** + `just preza-blocks{,-all}` / `just course-status`: план блоков записи из
  `content/presentations.yml → recording.blocks` (id слайдов) — полное покрытие, порядок, без
  наложений (ошибки), оценка минут и ⚠ блок > лимита (предупреждение; `--strict` — ошибка),
  `--md` для доков. Планы: dbt — 6 блоков (70 сл. ≈ 91 мин; «dbt Core» из предложения
  менеджера разбит надвое — 24 сл. ≈ 31 мин), Dagster — 4 блока (49 сл. ≈ 63.7 мин).
- SessionStart-хук **`scripts/hooks/course-production-status.sh`** (fail-open, без сети): дни
  до дедлайна записи, деки лектора без плана блоков, длинные блоки, число открытых вопросов.
- Сабагент **`.claude/agents/course-production-keeper.md`** — владелец ленты «чат менеджера →
  правила / Q&A / настройки / блоки»; `preza-accents-keeper` — инвариант 9 (перепроверять
  блоки после переносов слайдов, не перезатирать дизайн-пасс менеджера). Кандидат скилла
  `course-rules-upsert` (`~/.ai/skills/_skills_candidates_by_prj/MLInside-course/`).
- Тесты: `src/tests/test_course_blocks.py` (+19 → 280 passed, 4 skipped); ruff чист.
