...
- `just preza-merge-verify` на этом кейсе завершается с ненулевым кодом: структурная сверка
  перестроенной базовой колоды против форка находит остаточные расхождения геометрии сверх
  допуска (±0.4″) на части слайдов — адаптивная ширина колонки буллетов и высота
  код-панелей воспроизводят решение правила лишь приближённо, а не точную ручную подгонку
  каждого слайда; это ожидаемо для правил R2/R4 (их доля срабатывания < 1.0) и зафиксировано
  в отчёте, допуски/профиль под верификацию не подгонялись.
