...
## Триггер: tg — автоматически в конце хода, drive и sheet — явно

Билдер (`preza_gen`) мятит **новую minor-версию на каждом билде** (`_resolve_naming`
сканирует `data/generated/`), поэтому «отправлять на каждую сборку» — это спам: один
рабочий ход часто даёт три-четыре версии подряд, из которых показывать стоит последнюю.

Отсюда компромисс (2026-08-26, по запросу владельца — раньше лента была полностью явной).
Stop-хук `scripts/hooks/deck-publish-auto-send.sh` в конце хода вызывает
`just publish-new --only tg`. Спам это не порождает по двум причинам: публикатор видит
только **новейшую** версию деки (промежуточные билды того же хода не существуют для него),
а курсор держит правило «одна версия — одна отправка». Итог — не больше одного сообщения
на деку за ход.

Что осталось явным: **drive и sheet**. Их отказы — про квоту и права, чинятся человеком, и
автоматический ретрай тут ничего не даёт (`just publish-new --only drive --only sheet`).

Выключатель — `settings/publish.yml → telegram.auto_send: false`, без правки кода; тогда
лента снова полностью явная. Хук fail-open: нет новых версий — молчит; не смог прочитать
настройки — молчит; `stop_hook_active` — выходит сразу, чтобы не будить себя рекурсивно;
лок `data/.state/.auto-send.lock` не даёт двум одновременно завершившимся сессиям
отправить одно и то же дважды.

SessionStart-хук `scripts/hooks/deck-publish-status.sh` (fail-open, без сети) по-прежнему
напоминает о двух разных состояниях, не смешивая их:
...
- **TG**: текст через `tg-project-inform.sh --slug mlinside_course` (роутинг по слагу →
  ...
  лента, дублей нет. Это условие стало **жёстче** после включения авто-отправки: `deck-watch`
  шлёт файл мимо курсора, поэтому `on_complete: "open_send"` дал бы ровно те дубли, от
  которых защищает публикатор.
