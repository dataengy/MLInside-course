...
1. **Дедлайн менеджера: записать всё до 2026-08-31** (5 дней). Ни одна наша лекция не
   записана, пробный клип не сделан, ПО по инструкции не подтверждено.
2. Ответить Алине: баллы ДЗ (наши слайды и `HW*.md` в баллах), тест-запись, ориентир даты
   записи dbt — черновики ответов под чекбоксами в `docs/course-qa.md`.
3. Дизайн-правки Алины (шрифт / отступы / обводка код-блоков) живут только в её pptx
...
4. CI/CD-дека — без плана блоков (хук напоминает); черновик менеджеру не отправлялся.
