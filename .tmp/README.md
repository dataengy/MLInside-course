...
| `label_appendix_notes.py` | Ручная карта «абзац → раздел» для заметок PART 2, PART 3 и «Картинок»: они были свободной прозой и потому проходили мимо `notes_fix.py`. Сверяет поабзацно, что правка приписала ярлык и ничего больше. |
| `fold_continuations.py` | Разовая свёртка абзацев-продолжения в их раздел: без метки блок «проходит насквозь» мимо `notes_fix.py` и остаётся прозой даже со снятым порогом булитирования. Перед записью сверяет, что изменилась только разметка списка, а не слова. |
| `graft_slides.py` | Разовый перенос отдельных слайдов между двумя `.pptx`, выросшими из одного шаблона: `--insert S:AFTER`, `--replace S:T`, `--report`. Закрывает дыру, которую `preza_merge` оставляет намеренно — там backend `graft` не реализован, лента разбирает форматирование, а не переносит слайды. Макет ищется по ИМЕНИ и при несовпадении падает, а не подставляет чужой молча. |
| `v45/` + `v41_patch.py` | Перенос dagster-правок из ручной деки `v4.2.1-man`: слайд «А если мы используем Dagster?» разрезан на два (R12 не держит картинку + две панели), код переписан по документации Dagster. |
| `v46/` + `v41_patch.py` | Слайд «Короткий рейтинг» за таблицей интеграции dbt/Airflow — картинка, спасённая из той же ручной деки. |
| `v47/` + `v41_patch.py` | Шесть правок из снимка 19:37 ручной деки `v4.7.0+man`: перестановка вступления, текст слайда про AE, две авторские схемы, слайд «Структура dbt-проекта», кроп Dagster UI. |
