...
def _plain(text: str) -> str:
    ...
    Шапка (часы + длительность) нормализуется целиком, а длительность приписывается обратно
    отдельным полем. Иначе смена формата шапки — `00:00-01:00 [~1 мин]` на
    `[00:00–01:00 · ~1 мин]` — читалась бы как потеря текста, хотя это ровно та правка,
    ради которой скрипт и запускают. Само число при этом остаётся под присмотром: подменить
    длительность молча по-прежнему нельзя.
    ...
    >>> _plain("[00:00–01:00 · ~1 мин] **Главное:**\\n- Это раз.\\n- Ни один не два.")
    '~1 мин **главное:** это раз. ни один не два.'
    >>> _plain("[~1 мин] **Главное:** раз.") == _plain("[~2 мин] **Главное:** раз.")
    False
    >>> _plain("**Главное:** раз.") == _plain("**Главное:** два.")
    False
    """
    mins = N.duration(text)
    text = N.strip_head(text.strip())
    text = re.sub(r"^[ \t]*- ", "", text, flags=re.M)
    body = re.sub(r"\s+", " ", text).strip().lower()
    return (f"~{mins:g} мин " if mins is not None else "") + body
